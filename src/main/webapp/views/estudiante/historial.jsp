            ...
            <c:otherwise>
              <div class="table-responsive">
                <table class="table table-hover mb-0">
                  <thead class="table-light"><tr><th>Materia</th><th>Tutor</th><th>Día / Hora</th><th>Cubículo</th><th>Estado</th></tr></thead>
                  <tbody>
                    <c:forEach var="t" items="${historial}">
                      <tr>
                        <td class="fw-semibold small"><c:out value="${t.franja.materia.nombre}"/></td>
                        <td class="small"><c:out value="${t.franja.docente.usuario.nombreCompleto}"/></td>
                        <td class="small">${t.franja.diaSemana} ${t.franja.horaInicioStr}</td>
                        <td class="small">C-${t.franja.cubiculo}</td>
                        <td><span class="badge ${t.estado=='ACTIVA'?'text-bg-success':t.estado=='COMPLETADA'?'text-bg-primary':'text-bg-danger'}">${t.estado}</span></td>
                      </tr>
                    </c:forEach>
                  </tbody>
                </table>
